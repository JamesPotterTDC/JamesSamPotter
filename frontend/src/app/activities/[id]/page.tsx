...
export default async function ActivityDetailPage({
  ...
  const accent = indoor ? 'text-cyan-400' : 'text-orange-400';
  ...
  // Power zone context (relative to a rough FTP estimate or absolute thresholds)
  ...
  return (
    <div className="min-h-screen bg-void">
      ...
      {/* HERO — full-width map or indoor visual */}
      <div className={`relative w-full ${hasMap ? 'h-[55vh] min-h-[400px]' : 'h-[40vh] min-h-[320px]'}`}>
        {hasMap ? (
          <>
            ...
            />
            {/* Gradient overlay — title bleeds over map */}
          ...
          >
            {/* Indoor concentric rings */}
            {[120, 200, 280].map((r, i) => (
              <div
                key={r}
                className="absolute rounded-full border border-cyan-400/[0.06] animate-pulse-slow"
        ...
        <div className="absolute inset-x-0 bottom-0 max-w-7xl mx-auto px-6 pb-8">
          ...
          <h1 className="font-bebas text-4xl sm:text-5xl lg:text-6xl text-white tracking-tight leading-none">
            {activity.name}
          </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-10 pb-24">
        ...
        {story && (
          <FadeIn>
            ...
            >
              <p className="text-sm font-display text-slate-300 leading-relaxed italic">
                &ldquo;{story}&rdquo;
              </p>
            </div>
          </FadeIn>
                  ...
                  {activity.kilojoules && (
                    <div className="mt-3 pt-3 border-t border-white/[0.06]">
                      <p className="text-xs text-slate-600">
                        {formatEnergy(activity.kilojoules)} · {pints > 0 ? `${pints} pints earned` : 'keep pedalling'}
                      </p>
                    </div>
                  )}
                </div>
              ...
              {activity.average_heartrate && (
                <div className="card p-6">
                  ...
                  <div className="flex items-end gap-6 mb-4">
                    <div>
                      <p className="text-xs text-slate-600 mb-1">Avg</p>
                      <p className="font-bebas text-4xl leading-none" style={{ color: accentHex }}>
                    ...
                    {activity.max_heartrate && (
                      <div>
                        ...
                        <p className="font-bebas text-4xl text-slate-300 leading-none">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs font-medium text-slate-400">
                          {Math.round((activity.average_heartrate / activity.max_heartrate) * 100)}%
                        </span>
                      </div>
                      <div className="h-1 bg-white/[0.06] rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.round((activity.average_heartrate / activity.max_heartrate) * 100)}%`,
        ...
        <FadeIn delay={0.2}>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
            {activity.average_cadence && (
              <SmallStat label="Avg Cadence" value={`${Math.round(activity.average_cadence)} rpm`} />
            )}
            {activity.max_speed_mps && (
              <SmallStat label="Max Speed" value={formatPace(activity.max_speed_mps)} />
            )}
            {activity.kilojoules && (
              <SmallStat label="Energy" value={formatEnergy(activity.kilojoules)} />
            )}
        ...
        {activity.total_elevation_gain_m > 100 && (
          <FadeIn delay={0.25}>
            <div className="card p-5 flex items-center gap-4 mb-6">
              ...
              <div>
                <p className="font-bebas text-2xl text-violet-400 leading-none">
                  {everestMultiple(activity.total_elevation_gain_m)} Everest
                </p>
                <p className="text-xs text-slate-500 mt-0.5">
                  {formatElevation(activity.total_elevation_gain_m)} gained · Everest is 8,848 m
                </p>
              </div>
            </div>
          </FadeIn>
...
function HeroStat({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="card p-5">
      ...
      <p className="font-bebas text-3xl sm:text-4xl leading-none" style={{ color: accent }}>
        {value}
      </p>
    </div>
  );
